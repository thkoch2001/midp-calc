...
public class Guess
{
    ...
    public class GuessExpr
    {
        ...
        public void calcInformation() {
            ...
            //               (1<<entropyPenalty));
            // The 2*b-1 bit is a simple way of saying
            //   "if (b!=1) entropyPenalty++;"
            d.assign(origValue);
            eI.assign(value);
            eI.sub(d);
            eI.abs();
        ...
        public void calcProbability(Real prob) {
            ...
            if (prob.lessThan(Real.TENTH)) // No probability like 6.1e-14%
                prob.makeZero();
        }
    ...
    private void updateGuess(int a, int b) {
        ...
        tmp.sub(newGuess.value);// Sub with magic rounding ==0 if <4 bits diff
        ...
        if (tmp.isZero()) {
            ...
            if (newGuess.isBetterThan(firstGuess)) {
                ...
                newGuess.eI.add(firstGuess.eI);// Sum for increased confidence
                firstGuess.copy(newGuess);
            } else {
                firstGuess.eI.add(newGuess.eI);// Sum for increased confidence
            }
        }
...
//             StrPrintF(result+StrLen(result),
//                       "e^(%ldpi*i)",(Int32)round(tmp));
//             found = true;
//         }
