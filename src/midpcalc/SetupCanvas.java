    ...
    implements CommandListener
{
    ...
    public SetupCanvas(Calc m) {
        ...
        alertText = "To adapt the user interface, "+
        ...
        if (name != null) {
            ...
            if (name.startsWith("nokia")) {
                // Just start with the most likely arrangement for Nokia
                arrng = 1;
    ...
    private void setupKeys() {
        ...
        if (alertText != null) {
            ...
            if ((arrng & 0x80) == 0) {
                no  = new Command("no",
                                  commandArrangement[2*(arrng & 0x7f)], 1);
                yes = new Command("yes",
                                  commandArrangement[2*(arrng & 0x7f)+1], 1);
                addCommand(no);
                addCommand(yes);
            } else {
                yes = new Command("yes",
            ...
            if ((arrng & 0x80) == 0) {
                left = new Command("left",
                                   commandArrangement[2*(arrng & 0x7f)], 1);
                right= new Command("right",
                                   commandArrangement[2*(arrng & 0x7f)+1], 1);
                addCommand(left);
                addCommand(right);
            } else {
    ...
    protected void keyPressed(int key) {
        ...
        if (query == QUERY_FINISHED) {
            ...
            // Wait and see, this could be a double keyPressed/commandAction
            // event
                        ...
                        switch (key) {
                            ...
                            default:
                                if (midlet.doubleKeyEvents)
                                    // We don't yet know if we can treat this
                                    // as "clear"
                                    unknownKeyPressed = true;
                                else
                                    clearKeyPressed(true);
    ...
    protected void keyReleased(int key) {
        if (unknownKeyPressed) {
            ...
            if (query == COMMAND_QUERY) {
                ...
                // Unknown key has been pressed and released, treat as "clear"
                clearKeyPressed(true);
            }
        }
    }
