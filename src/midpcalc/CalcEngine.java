...
public final class CalcEngine
{
    ...
    private void financeSolve(int which) {
        ...
        switch (which)
        {
            ...
            case 4: // IR
                ...
                for (int n=0; n<60; n++) {
                    if (n>=2) {
                        if ((n&1) == 1 && Y1.sign != Y2.sign) {
                            // On odd n if the root is straddled,
                            // bisect instead of approximate:
                            rTmp.assign(X1);
                            rTmp.add(X2);
                            rTmp.scalbn(-1);
                        } else {
                    ...
                    if (n < 2 || Y1.sign != rTmp.sign || Y2.sign == rTmp.sign) {
                        X2.assign(X1);
                        Y2.assign(Y1);
                    }
                    X1.assign(IR);
                    Y1.assign(rTmp);
                    if (!rTmp.isFinite() || !IR.isFinite() ||
                        (n >= 30 && rTmp.exponent <= 0x40000000-16))
                        break;
                }
                if (!rTmp.isFinite() || rTmp.exponent > 0x40000000-16) {
    ...
    public void command(int cmd, int param) throws OutOfMemoryError {
        ...
        switch (cmd)
        {
            ...
            case VERSION: push(0x40000002,0x4851EB851EB851ECL); break;
